...
CRITICAL_FILES = [
    ...
    "/etc/hosts", "/etc/ld.so.preload",
]
WEBSHELL_SIGNATURES = [
    "eval(base64_decode", "system($_GET", "exec($_POST",
    "passthru(", "shell_exec(", "<?php system(", "<?php exec(",
    "import os; os.system", "subprocess.call", "__import__('os').system",
]
WEB_ROOTS = ["/var/www/html", "/var/www", "/srv/www", "/opt/app", "/home/ubuntu/public_html"]
SUID_SCAN_PATHS = ["/usr/local/bin", "/tmp", "/var/tmp", "/dev/shm"]
HOME_BASES = ["/root", "/home"]
TMP_SCAN_PATH = "/tmp"


@tool("File System Monitor")
def filesystem_monitor_tool(hours_back: str = "24") -> dict:
        ...
        try:
            ...
            if mtime > cutoff:
                detections.append({
                    ...
                    "severity": "CRITICAL",
                    "detail": f"Critical system file {filepath} modified in last {hours}h",
                })
                ...
                try:
                    ...
                    if is_suid and mtime > cutoff:
                        detections.append({
                            ...
                            "severity": "HIGH",
                            "detail": f"New SUID binary in {scan_path} — possible privilege escalation",
                        })
                except (PermissionError, OSError):
                    continue
        except (PermissionError, OSError) as e:
                ...
                try:
                    ...
                    if mtime > cutoff:
                        detections.append({
                            ...
                            "modified_at": mtime.isoformat(),
                            "severity": "HIGH",
                            "detail": f"SSH authorized_keys modified — possible backdoor key added",
                        })
                except (PermissionError, OSError):
                    continue
        except (PermissionError, OSError) as e:
    ...
    try:
        for fname in os.listdir(TMP_SCAN_PATH):
            fpath = os.path.join(TMP_SCAN_PATH, fname)
            try:
                fstat = os.stat(fpath)
                mtime = datetime.fromtimestamp(fstat.st_mtime)
                ...
                if is_executable and mtime > cutoff and os.path.isfile(fpath):
                    detections.append({
                        ...
                        "severity": "HIGH",
                        "detail": f"Executable {fpath} in /tmp — malware staging location",
                    })
            except (PermissionError, OSError):
                continue
