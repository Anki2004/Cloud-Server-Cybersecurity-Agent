...
def _parse_ss(output: str) -> list:
    connections = []
    for line in output.strip().split("\n")[1:]:
        parts = line.split()
        if len(parts) >= 5:
            connections.append({"state": parts[0], "local": parts[3], "remote": parts[4]})
    return connections


def _analyze_connections(connections: list) -> list:
    detections = []
    ...
    for conn in connections:
        ...
        if port in SUSPICIOUS_INBOUND_PORTS:
            detections.append({
                ...
                "port": port,
                "severity": "MEDIUM",
                "detail": f"Active connection to sensitive port {port} from {ip}",
            })
    for ip, count in remote_ip_counter.items():
        if count > 10 and ip not in ("127.0.0.1", "::1"):
            detections.append({
                ...
                "severity": "HIGH",
                "detail": f"High connection frequency ({count}) to {ip} — possible C2",
            })
    return detections


def _check_listening(output: str) -> list:
    detections = []
    for line in output.strip().split("\n")[1:]:
        parts = line.split()
        if len(parts) < 4:
        ...
        if port_match:
            ...
            if port in BACKDOOR_PORTS:
                detections.append({
                    ...
                    "port": port,
                    "severity": "CRITICAL",
                    "detail": f"Backdoor port {port} actively listening",
                })
    return detections


@tool("Network Traffic Monitor")
def network_monitor_tool(query: str = "scan") -> dict:
    ...
    and suspicious outbound connections. Pass 'scan' as input."""

    detections = []
    try:
        result = subprocess.run(["ss", "-tnp"], capture_output=True, text=True, timeout=10)
        connections = _parse_ss(result.stdout)
        detections.extend(_analyze_connections(connections))
    except Exception as e:
        logger.error(f"ss -tnp failed: {e}")

    try:
        result = subprocess.run(["ss", "-tlnp"], capture_output=True, text=True, timeout=10)
        detections.extend(_check_listening(result.stdout))
    except Exception as e:
    ...
    try:
        with open("/var/log/auth.log", "r", errors="ignore") as f:
            content = f.read()
        ip_counts = Counter(re.findall(r"from (\d+\.\d+\.\d+\.\d+)", content))
        for ip, count in ip_counts.most_common(10):
            if count > 20:
                detections.append({
                    ...
                    "detail": f"IP {ip} made {count} attempts — likely scan or brute force",
                })
    except FileNotFoundError:
        logger.warning("auth.log not found")
    except Exception as e:
        logger.error(f"Port scan detection error: {e}")

    return {"total_network_threats": len(detections), "detections": detections}